...
#elif CONFIG_IDF_TARGET_ESP32S2BETA
        _fb2->buf = NULL; // ESP32-S2 RAM too less
#endif
        if (!_fb2->buf) {
...
#define DATA_SIG_OFFSET (8)

static lldesc_t _dma = {0};

static void i2s_dma_start_tmp(void)
{
    ...
    _dma.eof = 1;
    _dma.owner = 1;
    _dma.buf = malloc(1024 * 3);
    _dma.empty = NULL;
    if (_dma.buf == NULL) printf("malloc fail\n");
    I2S0.in_link.addr = ((uint32_t)&_dma) & 0xfffff;
    I2S0.rx_eof_num = 1024;
...
static int  SIG_OFFSET = 8;

static void camera_pin_cfg(void)
{
    printf("camera pin cfg\n");
...
static void i2s_init()
{
    ...
    gpio_matrix_in(config->pin_d0, I2S0I_DATA_IN0_IDX + DATA_SIG_OFFSET, false);
    gpio_matrix_in(config->pin_d1, I2S0I_DATA_IN1_IDX + DATA_SIG_OFFSET, false);
    gpio_matrix_in(config->pin_d2, I2S0I_DATA_IN2_IDX + DATA_SIG_OFFSET, false);
    gpio_matrix_in(config->pin_d3, I2S0I_DATA_IN3_IDX + DATA_SIG_OFFSET, false);
    gpio_matrix_in(config->pin_d4, I2S0I_DATA_IN4_IDX + DATA_SIG_OFFSET, false);
    gpio_matrix_in(config->pin_d5, I2S0I_DATA_IN5_IDX + DATA_SIG_OFFSET, false);
    gpio_matrix_in(config->pin_d6, I2S0I_DATA_IN6_IDX + DATA_SIG_OFFSET, false);
    gpio_matrix_in(config->pin_d7, I2S0I_DATA_IN7_IDX + DATA_SIG_OFFSET, false);
    ...
    //                &_i2s_isr, NULL, &s_state->i2s_intr_handle);
    esp_intr_alloc(ETS_I2S0_INTR_SOURCE, 0, _i2s_isr, NULL, &s_state->i2s_intr_handle);
}
...
static void IRAM_ATTR i2s_start_bus()
{
    ...
    I2S0.int_ena.val = 0;
    I2S0.int_clr.val = ~0;
    I2S0.rx_eof_num = 32;//s_state->dma_sample_count;
